...
function(BuildWheel WHEEL_TARGET PY_VER PYTHON_EXECUTABLE CUDA_VER)
    ...
    add_custom_target(
        ...
        COMMAND sed -i -e "s|{file = \"requirements.txt\"}|{file = \"requirements_cuda${CUDA_VER}.txt\"}|" ${PYPROJECT_PATH}
        COMMAND sed -i -e "s|open(f\"{os.path.dirname(__file__)}/requirements.txt\")|open(f\"{os.path.dirname(__file__)}/requirements_cuda${CUDA_VER}.txt\")|" ${SETUP_PATH}
        COMMAND mkdir -p ${BUILD_DIR}
        COMMAND echo "Building whl and tgz for packages ${CUDA_VER}"
        COMMAND ${CMAKE_COMMAND} -E env "CPPFLAGS=-I${CUDA_HOME}/include/" "PACKAGE_NAME=${WHEEL_NAME}" ${VENV_PYTHON_EXECUTABLE} -m build --outdir ${BUILD_DIR}
        ...
        COMMAND sed -i -e "s|{file = \"requirements_cuda${CUDA_VER}.txt\"}|{file = \"requirements.txt\"}|" ${PYPROJECT_PATH}
        COMMAND sed -i -e "s|open(f\"{os.path.dirname(__file__)}/requirements_cuda${CUDA_VER}.txt\")|open(f\"{os.path.dirname(__file__)}/requirements.txt\")|" ${SETUP_PATH}
