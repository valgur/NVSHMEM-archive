...
#define CALL_COLLECT(BITS)                                                                         \
    __global__ void test_collect##BITS##_call_kern(void *dest, const void *source, size_t nelems,  \
        ...
        if (!blockIdx.x && !threadIdx.x && nelems < 512) {                                         \
            for (i = 0; i < (iter + skip); i++) {                                                  \
                ...
                nvshmem_collect##BITS(dest, source, nelems, PE_start, logPE_stride, PE_size,       \
                                      pSync);                                                      \
        ...
        if (!blockIdx.x && !(threadIdx.x / warpSize) && nelems < 4096) {                                 \
            for (i = 0; i < (iter + skip); i++) {                                                  \
                ...
                if (i > skip) start = clock64();                                                   \
                nvshmemx_collect##BITS##_warp(dest, source, nelems, PE_start, logPE_stride,        \
                                              PE_size, pSync);                                     \
                if (i > skip) stop = clock64();                                                    \
                time += (stop - start);                                                            \
            }                                                                                      \
        ...
        if (!blockIdx.x) {                                                                         \
            for (i = 0; i < (iter + skip); i++) {                                                  \
                ...
                if (i > skip) start = clock64();                                                   \
                nvshmemx_collect##BITS##_block(dest, source, nelems, PE_start, logPE_stride,       \
                                               PE_size, pSync);                                    \
                if (i > skip) stop = clock64();                                                    \
                time += (stop - start);                                                            \
            }                                                                                      \
...
int collect_calling_kernel(void *dest, const void *source, int mype, int PE_start, int logPE_stride,
                           int PE_size, long *pSync, cudaStream_t stream, double *d_time_avg) {
    int status = 0;
    ...
    for (num_elems = 1; num_elems < max_elems; num_elems *= 2) {
        test_collect32_call_kern<<<num_blocks, nvshm_test_num_tpb, 0, stream>>>(
    ...
    if (!mype) printf("+--------------+-----------------+---------------+----------------+\n");
    for (num_elems = 1; num_elems < max_elems; num_elems *= 2) {
        test_collect64_call_kern<<<num_blocks, nvshm_test_num_tpb, 0, stream>>>(
            dest, source, num_elems, PE_start, logPE_stride, PE_size, pSync, mype, d_time_avg);
        cuda_check_error();
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_dest, h_dest, (sizeof(DATATYPE) * num_elems * npes),
                               cudaMemcpyHostToDevice, cstrm));
    CUDA_CHECK(cudaMemcpyAsync(d_pSync, h_pSync, (sizeof(long) * NVSHMEM_COLLECT_SYNC_SIZE),
                               cudaMemcpyHostToDevice, cstrm));

    collect_calling_kernel(d_dest, d_source, mype, 0 /*PE_start*/, 0 /*logPE_stride*/, npes,
    ...
    CUDA_CHECK(cudaMemcpyAsync(h_dest, d_dest, (sizeof(DATATYPE) * num_elems * npes),
                               cudaMemcpyDeviceToHost, cstrm));
    CUDA_CHECK(cudaMemcpyAsync(h_pSync, d_pSync, (sizeof(long) * NVSHMEM_COLLECT_SYNC_SIZE),
                               cudaMemcpyDeviceToHost, cstrm));
