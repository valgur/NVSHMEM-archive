...
__global__ void test_barrier_call_kern(int PE_start, int logPE_stride, int PE_size, long *pSync,
                                       int mype, double *d_time_avg) {
    int iter = BARRIER_MAX_ITERS;
    ...
    if (!blockIdx.x && !(threadIdx.x / warpSize)) {
        for (i = 0; i < (iter + skip); i++) {
            if (i > skip) start = clock64();
            nvshmemx_barrier_warp(PE_start, logPE_stride, PE_size, pSync);
            if (i > skip) stop = clock64();
            time += (stop - start);
        }
        if (!threadIdx.x) {
    ...
    if (!blockIdx.x) {
        for (i = 0; i < (iter + skip); i++) {
            if (i > skip) start = clock64();
            nvshmemx_barrier_block(PE_start, logPE_stride, PE_size, pSync);
            if (i > skip) stop = clock64();
            time += (stop - start);
        }
        if (!threadIdx.x) {
...
__global__ void test_barrier_all_call_kern(int PE_start, int logPE_stride, int PE_size, long *pSync,
                                           int mype, double *d_time_avg) {
    int iter = BARRIER_MAX_ITERS;
    int skip = BARRIER_MAX_SKIP;
    long long int start = 0, stop = 0;
...
int barrier_calling_kernel(int PE_start, int logPE_stride, int PE_size, long *pSync,
    ...
    if (!mype)
        printf("Latency of thread/warp/block variants of barrier and barrier_all API in us\n");
    if (!mype) printf("#barrier latency\n");
    if (!mype) printf("+-----------------+---------------+----------------+\n");
    if (!mype) printf("|   thread (us)   |   warp (us)   |   block (us)   |\n");
    if (!mype) printf("+-----------------+---------------+----------------+\n");
    nvshmem_barrier_all();
    test_barrier_call_kern<<<num_blocks, nvshm_test_num_tpb, 0, stream>>>(
        PE_start, logPE_stride, PE_size, pSync, mype, d_time_avg);
    cuda_check_error();
    CUDA_CHECK(cudaStreamSynchronize(stream));
    if (!mype) printf("+-----------------+---------------+----------------+\n");

    if (!mype) printf("#barrier_all latency\n");
    if (!mype) printf("+-----------------+---------------+----------------+\n");
    if (!mype) printf("|   thread (us)   |   warp (us)   |   block (us)   |\n");
    if (!mype) printf("+-----------------+---------------+----------------+\n");

    nvshmem_barrier_all();
    test_barrier_all_call_kern<<<num_blocks, nvshm_test_num_tpb, 0, stream>>>(
        PE_start, logPE_stride, PE_size, pSync, mype, d_time_avg);
    cuda_check_error();
    CUDA_CHECK(cudaStreamSynchronize(stream));
    if (!mype) printf("+-----------------+---------------+----------------+\n");
...
int main(int argc, char **argv) {
    ...
    barrier_calling_kernel(PE_start, logPE_stride, npes, d_pSync, cstrm, mype, d_time_avg);
