...
#define CALL_BCAST(BITS)                                                                           \
    __global__ void test_bcast##BITS##_call_kern(                                                  \
        void *dest, const void *source, int nelems, int PE_root, int PE_start, int logPE_stride,   \
        int PE_size, long *pSync, int mype, double *d_time_avg) {                                  \
        int iter = MAX_ITERS;                                                                      \
        int skip = MAX_SKIP;                                                                       \
        ...
        if (!blockIdx.x && !threadIdx.x && nelems < 512) {                                         \
            for (i = 0; i < (iter + skip); i++) {                                                  \
                ...
                nvshmem_broadcast##BITS(dest, source, nelems, PE_root, PE_start, logPE_stride,     \
                                        PE_size, pSync);                                           \
        ...
        if (!blockIdx.x && !(threadIdx.x / warpSize) && nelems < 4096) {                                 \
            for (i = 0; i < (iter + skip); i++) {                                                  \
                ...
                if (i > skip) start = clock64();                                                   \
                nvshmemx_broadcast##BITS##_warp(dest, source, nelems, PE_root, PE_start,           \
...
int broadcast_calling_kernel(void *dest, const void *source, int mype, int PE_root, int PE_start,
    ...
    for (num_elems = 1; num_elems < max_elems; num_elems *= 2) {
        test_bcast32_call_kern<<<num_blocks, nvshm_test_num_tpb, 0, stream>>>(
            dest, source, num_elems, PE_root, PE_start, logPE_stride, PE_size, pSync, mype,
            d_time_avg);
        cuda_check_error();
    ...
    if (!mype) printf("+--------------+-----------------+---------------+----------------+\n");
    for (num_elems = 1; num_elems < max_elems; num_elems *= 2) {
        test_bcast64_call_kern<<<num_blocks, nvshm_test_num_tpb, 0, stream>>>(
            dest, source, num_elems, PE_root, PE_start, logPE_stride, PE_size, pSync, mype,
            d_time_avg);
        cuda_check_error();
        CUDA_CHECK(cudaStreamSynchronize(stream));
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_dest, h_dest, (sizeof(DATATYPE) * num_elems),
                               cudaMemcpyHostToDevice, cstrm));
    CUDA_CHECK(cudaMemcpyAsync(d_pSync, h_pSync, (sizeof(long) * NVSHMEM_BCAST_SYNC_SIZE),
                               cudaMemcpyHostToDevice, cstrm));

    broadcast_calling_kernel(d_dest, d_source, mype, root, PE_start, logPE_stride, npes, d_pSync,
                             cstrm, d_time_avg);
    ...
    CUDA_CHECK(cudaMemcpyAsync(h_dest, d_dest, (sizeof(DATATYPE) * num_elems),
                               cudaMemcpyDeviceToHost, cstrm));
    CUDA_CHECK(cudaMemcpyAsync(h_pSync, d_pSync, (sizeof(long) * NVSHMEM_BCAST_SYNC_SIZE),
                               cudaMemcpyDeviceToHost, cstrm));
