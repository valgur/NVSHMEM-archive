...
int main(int argc, char **argv) {
    ...
    int i = 0;
    size_t size =
        (MAX_ELEMS * MAX_NPES * 2 * sizeof(DATATYPE)) + (NVSHMEM_ALLTOALL_SYNC_SIZE * sizeof(long));
    size_t alloc_size;
    int num_elems;
    ...
    alloc_size =
        (num_elems * npes * 2 * sizeof(DATATYPE)) + (NVSHMEM_ALLTOALL_SYNC_SIZE * sizeof(long));
    ...
    if (!mype) printf("+--------------+------------------+\n");
    RUN_COLL_ON_STREAM(alltoall, ALLTOALL, int, 32, d_source, h_source, d_dest, h_dest, d_pSync,
                       h_pSync, npes, -1, stream);
    ...
    if (!mype) printf("| size (bytes) |   latency (us)   |\n");
    if (!mype) printf("+--------------+------------------+\n");
    RUN_COLL_ON_STREAM(alltoall, ALLTOALL, long, 64, d_source, h_source, d_dest, h_dest, d_pSync,
                       h_pSync, npes, -1, stream);
