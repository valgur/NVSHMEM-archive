...
int main(int argc, char **argv) {
    ...
    RUN_COLL_ON_STREAM(broadcast, BCAST, int32, int32_t, (int32_t *)d_source, (int32_t *)h_source, 
                       (int32_t *)d_dest, (int32_t *)h_dest, npes, PE_root, stream, size_array, latency_array);
    if (!mype) {
        print_table("broadcast_on_stream", "32-bit", "size (bytes)", "latency", "us", '-', size_array, latency_array, MAX_ELEMS_LOG + 1);
    }

    RUN_COLL_ON_STREAM(broadcast, BCAST, int64, int64_t, d_source, h_source, d_dest, h_dest, npes, PE_root, stream, size_array, latency_array);
    if (!mype) {
        print_table("broadcast_on_stream", "64-bit", "size (bytes)", "latency", "us", '-', size_array, latency_array, MAX_ELEMS_LOG + 1);
    }
