...
int main(int argc, char **argv) {
    ...
    RUN_COLL(fcollect, FCOLLECT, int32, int32_t, (int32_t *)d_source, (int32_t *)h_source,
             (int32_t *)d_dest, (int32_t *)h_dest, npes, -1, stream, size_array, latency_array);
    if (!mype) {
        print_table("fcollect", "32-bit", "size (bytes)", "latency", "us", '-', size_array,
                    latency_array, MAX_ELEMS_LOG + 1);
    }

    RUN_COLL(fcollect, FCOLLECT, int64, int64_t, d_source, h_source, d_dest, h_dest, npes, -1,
             stream, size_array, latency_array);
    if (!mype) {
        print_table("fcollect", "64-bit", "size (bytes)", "latency", "us", '-', size_array,
                    latency_array, MAX_ELEMS_LOG + 1);
    }
