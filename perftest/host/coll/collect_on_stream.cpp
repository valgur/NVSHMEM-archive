...
int main(int argc, char **argv) {
    ...
    if (!mype) printf("+--------------+------------------+\n");
    RUN_COLL_ON_STREAM(collect, COLLECT, int, 32, d_source, h_source, d_dest, h_dest, d_pSync,
    ...
    if (!mype) printf("| size (bytes) |   latency (us)   |\n");
    if (!mype) printf("+--------------+------------------+\n");
    RUN_COLL_ON_STREAM(collect, COLLECT, long, 64, d_source, h_source, d_dest, h_dest, d_pSync,
                       h_pSync, npes, -1, stream);
