...
//#define NVSHMEM_GPUINITIATED_DEBUG
//#define NVSHMEM_TIMEOUT_DEVICE_POLLING

#define NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_MFENCE

#ifdef NVSHMEM_GPUINITIATED_SUPPORT_GPUMEM_ONLY
    // These PTX optimizations are for GPU memory access only.
    // Both data and NIC control objects must be in GPU memory.
    #define NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_ATOMIC_READ_SET
    #define NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_STORE_RELEASE
#endif
...
#ifdef NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_MFENCE
    asm volatile("fence.acq_rel.cta;" ::: "memory");
#else
    __threadfence_block();
#endif /* NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_MFENCE */
}
...
#endif /* NVSHMEM_GPUINITIATED_SUPPORT_GPUMEM_ONLY */
}

__device__ static inline void GIC_MEMBAR() {
// st.release automatically adds membar in SASS.
#ifndef NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_STORE_RELEASE
...
#endif /* NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_STORE_RELEASE */
}
...
#ifdef NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_STORE_RELEASE
    uint16_t _val = val;
...
#ifdef NVSHMEMI_GPUINITIATED_PTX_OPTIMIZATION_ATOMIC_READ_SET
    uint16_t ret;
...
__device__ static inline void gic_lock_release(int *lock) {
    ...
    if (nvshmemi_thread_id_in_threadgroup<SCOPE>() == 0)
        gic_atomic_set(lock, 0);
}

// Multiple threads may update get_head concurrently.
// Only the latest one w.r.t. wqe_idx is important.
__device__ static inline void gic_update_get_head(nvshmemi_gic_device_dci_t *dci, uint64_t new_get_head) {
    atomicMax((unsigned long long int *)&dci->mvars->tx_wq.get_head, (unsigned long long int)new_get_head);
}

__device__ static inline void gic_update_get_tail(nvshmemi_gic_device_dci_t *dci, uint64_t new_get_tail) {
    atomicMax((unsigned long long int *)&dci->mvars->tx_wq.get_tail, (unsigned long long int)new_get_tail);
}

__device__ static inline void *gic_get_wqe_ptr(nvshmemi_gic_device_dci_t *dci, uint16_t wqe_idx) {
    uint16_t cnt = dci->tx_wq.nwqes;
    uint16_t idx = wqe_idx & (cnt - 1); 
    return (void *)((uintptr_t)dci->tx_wq.wqe + (idx << MLX5_SEND_WQE_SHIFT));
}
    ...
    if (support_half_av_seg) {
        switch (sop) {
            ...
            case MLX5_OPCODE_ATOMIC_MASKED_FA:
                return 2;
#ifndef NVSHMEM_GPUINITIATED_DEBUG
...
__device__ static inline int gic_poll_cq(nvshmemi_gic_device_cq_t *cq, uint64_t idx, int *error) {
    ...
    do {
        ...
        if (unlikely(now - start > GIC_POLL_TIMEOUT)) {
            *error = -ETIME;
            printf("[%d] gic_poll_cq timeout because of MLX5_CQE_INVALID:\n"
                   "    cons_tail=%#lx, cons_head=%#lx, cqn=%#x\n"
                   "    qpn=%#x\n"
                   "    wqe_head=%#lx, wqe_tail=%#lx\n"
                   "    while waiting for idx=%#lx.\n",
                   nvshmemi_device_state_d.mype,
                   cons_tail, gic_atomic_read(cq->cons_head), cq->cqn,
                   cq->qpn,
                   gic_atomic_read(cq->wqe_head), gic_atomic_read(cq->wqe_tail),
                   idx
            );
            status = -1;
            goto out;
    ...
    GIC_MFENCE();
#endif /* NVSHMEM_GPUINITIATED_DEBUG */
    ...
    do {
        ...
        sop = chunk_data >> 56;
        polled_cons_tail = wqe_counter + gic_sop_to_nwqes<support_half_av_seg>(sop);
        ...
        if (cons_tail_lo != polled_cons_tail) {
            ...
            // We might read corrupted wqe_counter because NIC keeps updating it.
            // But we can trust it if the calculated cons_tail == cons_head.
            // Note that cons_tail < cons_head cannot be used because of potential corrupted value.
            if (cons_tail == cons_head) {
                // Other threads might update cons_tail concurrently.
                atomicMax((unsigned long long int *)cq->cons_tail, (unsigned long long int)cons_tail);
#ifdef NVSHMEM_TIMEOUT_DEVICE_POLLING
        ...
        if (unlikely(now - start > GIC_POLL_TIMEOUT)) {
            *error = -ETIME;
#ifdef NVSHMEM_GPUINITIATED_DEBUG
            printf("[%d] gic_poll_cq timeout:\n"
                   "    cons_tail=%#lx, cons_head=%#lx, chunk_data=%#lx, cqn=%#x\n"
                   "    polled_cons_tail=%#x, qpn=%#x\n"
                   "    wqe_head=%#lx, wqe_tail=%#lx\n"
                   "    while waiting for idx=%#lx.\n",
                   nvshmemi_device_state_d.mype,
                   cons_tail, cons_head, chunk_data, cq->cqn,
                   polled_cons_tail, cq->qpn,
                   gic_atomic_read(cq->wqe_head), gic_atomic_read(cq->wqe_tail),
                   idx
            );
#endif /* NVSHMEM_GPUINITIATED_DEBUG */
...
__device__ static inline void gic_write_dump_wqe(
    nvshmemi_gic_device_dci_t *dci
    , uint64_t laddr, __be32 lkey, uint32_t bytes
    , uint16_t wqe_idx, void **out_wqes, gic_ctrl_seg_t *out_ctrl_seg) {

    gic_ctrl_seg_t ctrl_seg;
    struct mlx5_wqe_data_seg data_seg;

    gic_ctrl_seg_t *ctrl_seg_ptr = (gic_ctrl_seg_t *)out_wqes[0];
    struct mlx5_wqe_data_seg *data_seg_ptr = (struct mlx5_wqe_data_seg *)((uintptr_t)out_wqes[0] + sizeof(*ctrl_seg_ptr));
    ...
    ctrl_seg = {0,};
    ctrl_seg.qpn_ds = HTOBE32((dci->qpn << 8) | 2);
...
template <bool support_half_av_seg>
__device__ static inline void gic_write_rdma_write_wqe(
...
__device__ static inline void gic_write_rdma_write_inl_wqe(
    ...
    inl_seg_ptr = (struct mlx5_wqe_inl_data_seg *)((uintptr_t)raddr_seg_ptr + sizeof(*raddr_seg_ptr));
...
__device__ static inline void gic_write_rdma_write_inl_wqe_combine_warp(
    nvshmemi_gic_device_dci_t *dci, nvshmemi_gic_device_dct_t *dct
    , const T val, uint64_t raddr, __be32 rkey
    , uint16_t wqe_idx, int my_tid
    ...
    size_t av_seg_size;
    int ds = gic_get_ds_in_inl_combine_warp<T>();

    uint32_t bytes = sizeof(T);

    // DC supports up to 16 DS WQE
    assert(likely(bytes <= 4));
    ...
    uint32_t my_data_idx = my_tid * bytes;
    if (my_data_idx < 12)
        wqe_data_ptr = (T *)((uintptr_t)inl_seg_ptr + sizeof(*inl_seg_ptr) + my_data_idx);
    else {
        my_data_idx -= 12;
    ...
    uint32_t nop_relative_wqe_idx = gic_get_num_wqes_in_inl_combine_warp<T>() - 1;
...
template <bool support_half_av_seg>
__device__ static inline void gic_write_rdma_read_wqe(
    nvshmemi_gic_device_dci_t *dci, nvshmemi_gic_device_dct_t *dct
    , uint64_t laddr, __be32 lkey, uint64_t raddr, __be32 rkey, uint32_t bytes
    , uint16_t wqe_idx, void **out_wqes, gic_ctrl_seg_t *out_ctrl_seg) {
...
__device__ static inline void gic_write_atomic_wqe(
    ...
    , uint16_t wqe_idx, nvshmemi_amo_t amo_op, void **out_wqes
    ...
    if (support_half_av_seg) {
        ...
        av_seg_size = sizeof(gic_half_av_seg_t);
        raddr_seg_ptr = (struct mlx5_wqe_raddr_seg *)((uintptr_t)av_seg_ptr + av_seg_size);
        atomic_seg_1_ptr = (struct mlx5_wqe_atomic_seg *)((uintptr_t)raddr_seg_ptr + sizeof(*raddr_seg_ptr));
        ...
        raddr_seg_ptr = (struct mlx5_wqe_raddr_seg *)out_wqes[1];
        atomic_seg_1_ptr = (struct mlx5_wqe_atomic_seg *)((uintptr_t)raddr_seg_ptr + sizeof(*raddr_seg_ptr));
        atomic_seg_2_ptr = (struct mlx5_wqe_atomic_seg *)((uintptr_t)atomic_seg_1_ptr + sizeof(*atomic_seg_1_ptr));
    }
    ...
    switch (amo_op) {
        ...
        case NVSHMEMI_AMO_INC: {
            ctrl_seg.qpn_ds = HTOBE32((dci->qpn << 8) | ds);
            if (bytes == 4) {
                ctrl_seg.opmod_idx_opcode = HTOBE32(MLX5_OPCODE_ATOMIC_MASKED_FA | (wqe_idx << 8) | GIC_4_BYTE_EXT_AMO_OPMOD);

                gic_atomic_32_masked_fa_seg_t *atomic_32_masked_fa_seg = (gic_atomic_32_masked_fa_seg_t *)&atomic_seg_1;
                atomic_32_masked_fa_seg->add_data = HTOBE32((uint32_t)1);
                atomic_32_masked_fa_seg->field_boundary = 0;
            } else {
                ctrl_seg.opmod_idx_opcode = HTOBE32(MLX5_OPCODE_ATOMIC_MASKED_FA | (wqe_idx << 8) | GIC_8_BYTE_EXT_AMO_OPMOD);

                gic_atomic_64_masked_fa_seg_t *atomic_64_masked_fa_seg = (gic_atomic_64_masked_fa_seg_t *)&atomic_seg_1;
        ...
        case NVSHMEMI_AMO_SET: {
            if (bytes == 4) {
                ctrl_seg.qpn_ds = HTOBE32((dci->qpn << 8) | ds);
                ctrl_seg.opmod_idx_opcode = HTOBE32(MLX5_OPCODE_ATOMIC_MASKED_CS | (wqe_idx << 8) | GIC_4_BYTE_EXT_AMO_OPMOD);

                gic_atomic_32_masked_cs_seg_t *atomic_32_masked_cs_seg = (gic_atomic_32_masked_cs_seg_t *)&atomic_seg_1;
                ...
                ctrl_seg.qpn_ds = HTOBE32((dci->qpn << 8) | (ds + 1));
                ctrl_seg.opmod_idx_opcode = HTOBE32(MLX5_OPCODE_ATOMIC_MASKED_CS | (wqe_idx << 8) | GIC_8_BYTE_EXT_AMO_OPMOD);

                gic_atomic_64_masked_cs_seg_t *atomic_64_masked_cs_data_seg = (gic_atomic_64_masked_cs_seg_t *)&atomic_seg_1;
                atomic_64_masked_cs_data_seg->swap = HTOBE64(*(uint64_t *)val_1);
                atomic_64_masked_cs_data_seg->compare = 0;

                gic_atomic_64_masked_cs_seg_t *atomic_64_masked_cs_mask_seg = (gic_atomic_64_masked_cs_seg_t *)&atomic_seg_2;
                atomic_64_masked_cs_mask_seg->swap = UINT64_MAX;
                atomic_64_masked_cs_mask_seg->compare = 0;

                data_seg_ptr = (struct mlx5_wqe_data_seg *)((uintptr_t)atomic_seg_2_ptr + sizeof(*atomic_64_masked_cs_mask_seg));
            }
            break;
        }
...
__device__ static inline void gic_update_dbr(nvshmemi_gic_device_dci_t *dci, uint32_t dbrec_head) {
...
__device__ static inline void gic_ring_db(nvshmemi_gic_device_dci_t *dci, gic_ctrl_seg_t *ctrl_seg) {
...
__device__ static inline void gic_post_send(nvshmemi_gic_device_dci_t *dci, uint64_t new_cons_idx, gic_ctrl_seg_t *ctrl_seg) {
    uint64_t old_cons_idx;

    // Update cons_head before ringing the db so that we know which index is needed in quiet/fence.
    gic_lock_acquire<NVSHMEMI_THREADGROUP_THREAD>(&dci->mvars->post_send_lock);

    if (need_strong_flush)
        old_cons_idx = atomicMax((unsigned long long int *)&dci->mvars->tx_wq.cons_head, (unsigned long long int)new_cons_idx);
    else
        old_cons_idx = atomicMax_block((unsigned long long int *)&dci->mvars->tx_wq.cons_head, (unsigned long long int)new_cons_idx);

    if (likely(new_cons_idx > old_cons_idx)) {
        GIC_MEMBAR();
        gic_update_dbr(dci, new_cons_idx);
        GIC_MEMBAR();
        gic_ring_db(dci, ctrl_seg);
    }

    gic_lock_release<NVSHMEMI_THREADGROUP_THREAD>(&dci->mvars->post_send_lock);
}

// If `dci` is shared among CTAs, need_strong_flush must be set to true because
// we must push prior writes from this CTA to L2 before coalescing DB.
template <bool need_strong_flush>
__device__ static inline void gic_submit_requests(nvshmemi_gic_device_dci_t *dci, uint64_t base_wqe_idx, 
    uint16_t num_wqes, gic_ctrl_seg_t *ctrl_seg) {
    ...
    if (need_strong_flush)
        while (atomicCAS((unsigned long long int *)&dci->mvars->tx_wq.wqe_tail, (unsigned long long int)base_wqe_idx, (unsigned long long int)new_wqe_idx) != base_wqe_idx) ; // wait here
    else
        while (atomicCAS_block((unsigned long long int *)&dci->mvars->tx_wq.wqe_tail, (unsigned long long int)base_wqe_idx, (unsigned long long int)new_wqe_idx) != base_wqe_idx) ; // wait here

    GIC_MFENCE();

    bool do_post_send = (new_wqe_idx == gic_atomic_read(&dci->mvars->tx_wq.wqe_head)) // No concurrent submissions
        || ((base_wqe_idx & mask) != (new_wqe_idx & mask)) // Num of not-yet-posted wqes is beyond the threshold.
        || (num_wqes >= state->num_requests_in_batch); // The number of wqes in this submission reaches the threshold.

    if (do_post_send)
        gic_post_send<need_strong_flush>(dci, new_wqe_idx, ctrl_seg);
}

template <bool support_half_av_seg>
__device__ static inline uint64_t gic_quiet(nvshmemi_gic_device_dci_t *dci) {
    uint64_t wqe_tail = gic_atomic_read(&dci->mvars->tx_wq.wqe_tail);
...
template <bool support_half_av_seg>
__device__ static inline void gic_wait_for_slot_availability(nvshmemi_gic_device_dci_t *dci, uint64_t wqe_idx) {
    // Don't have to use READ_ONCE here.
    // Even if we get stale cons_tail, poll_cq will catch that.
    int status = 0;
    int err = 0;
    uint64_t cons_tail = dci->mvars->tx_wq.cons_tail;
...
__device__ static inline nvshmemi_gic_device_dct_t *gic_get_dct(int pe) {
    ...
    if (dct_idx < NVSHMEMI_GIC_MAX_CONST_DCTS)
        return &state->constmem.dcts[dct_idx];
    
    return &state->globalmem.dcts[dct_idx - NVSHMEMI_GIC_MAX_CONST_DCTS];
}

__device__ static inline nvshmemi_gic_device_dci_t *gic_get_dci(int pe, bool *out_shared_among_ctas) {
    nvshmemi_gic_device_state_t *state = gic_get_state();
    uint32_t id;
    bool shared_among_ctas = false;
    uint32_t warpid = nvshmemi_thread_id_in_block() / nvshmemi_warp_size();
    switch (state->dci_map_type) {
        case NVSHMEMI_GIC_DEVICE_DCI_MAP_TYPE_CTA:
            id = gic_get_ctaid();
            break;
        case NVSHMEMI_GIC_DEVICE_DCI_MAP_TYPE_SM:
            id = gic_get_smid();
            shared_among_ctas = true;
            break;
        case NVSHMEMI_GIC_DEVICE_DCI_MAP_TYPE_WARP:
            id = gic_get_ctaid() * nvshmemi_block_size() + warpid;
            break;
        case NVSHMEMI_GIC_DEVICE_DCI_MAP_TYPE_DCT: {
            uint32_t dct_id = gic_get_dct_id(pe);
            uint32_t group_id = gic_get_ctaid() * nvshmemi_block_size() + warpid;
            id = (group_id % state->num_dct_groups) * state->ndcts_per_pe * nvshmemi_device_state_d.npes + dct_id;
            shared_among_ctas = true;
            break;
        }
        default:
...
__device__ static inline __be32 gic_get_lkey(uint64_t addr) {
    ...
    if (heap_start <= addr && addr <= heap_end) {
        ...
        uint64_t idx = 0;   // Without VMM, the symmetric heap has one lkey.
        if (state->use_cuda_vmm)
            idx = (addr - heap_start) >> state->log2_cumem_granularity;

        if (idx < NVSHMEMI_GIC_MAX_CONST_LKEYS)
            return state->constmem.lkeys[idx];

        return state->globalmem.lkeys[idx - NVSHMEMI_GIC_MAX_CONST_LKEYS];
    } else {
        // local-only addr
        nvshmemi_gic_device_local_only_mhandle_t *mhandle = state->globalmem.local_only_mhandle_head;
        while (mhandle) {
...
__device__ static inline void gic_get_raddr_rkey(uint64_t addr, int pe, uint64_t *raddr, __be32 *rkey) {
    nvshmemi_gic_device_state_t *state = gic_get_state();
    uint64_t heap_start = (uint64_t)nvshmemi_device_state_d.heap_base;
    uint64_t roffset = addr - heap_start;
    uint64_t idx = pe;  // Without VMM, the symmetric heap has one rkey per pe.
    if (state->use_cuda_vmm)
        idx = ((roffset >> state->log2_cumem_granularity) * nvshmemi_device_state_d.npes) + pe;
...
template <bool support_half_av_seg>
__device__ static inline uint64_t gic_reserve_wqe_slots(nvshmemi_gic_device_dci_t *dci, unsigned long long int num_wqes) {
    uint64_t wqe_idx = atomicAdd((unsigned long long int *)&dci->mvars->tx_wq.wqe_head, num_wqes);
    // If last slot is available, all prior slots are also available.
    gic_wait_for_slot_availability<support_half_av_seg>(dci, wqe_idx + num_wqes);
    return wqe_idx;
}

__device__ static inline uint64_t gic_reserve_ibuf_slots(nvshmemi_gic_device_dci_t *dci, unsigned long long int num_slots) {
    uint32_t nslots = dci->ibuf.nslots;
    uint64_t base_idx = atomicAdd((unsigned long long int *)&dci->mvars->ibuf.head, num_slots);
    uint64_t idx = base_idx + num_slots;

    // Wait until the slots become available.
    while (idx - gic_atomic_read(&dci->mvars->ibuf.tail) > nslots) ;
...
__device__ static inline void gic_release_ibuf(nvshmemi_gic_device_dci_t *dci, unsigned long long int base_idx, unsigned long long int num_slots) {
    unsigned long long int new_idx = base_idx + num_slots;
    GIC_MFENCE();
    // Wait here.
    while (atomicCAS((unsigned long long int *)&dci->mvars->ibuf.tail, (unsigned long long int)base_idx, new_idx) != base_idx) ;
    GIC_MFENCE();
}

__device__ static inline uint64_t gic_get_ibuf_addr(nvshmemi_gic_device_dci_t *dci, uint64_t idx) {
    idx = idx & (dci->ibuf.nslots - 1);

    // buf[0] is reserved for non-fetch operations
    return (uint64_t)dci->ibuf.buf + NVSHMEMI_GIC_IBUF_SLOT_SIZE * (idx + 1);
}

template <bool support_half_av_seg, bool nic_buf_on_gpumem>
__device__ static inline uint64_t gic_cst(nvshmemi_gic_device_dci_t *dci, bool is_dci_shared_among_ctas) {
...
#ifndef NVSHMEM_GPUINITIATED_SUPPORT_GPUMEM_ONLY
    ...
    uint64_t base_wqe_idx = gic_reserve_wqe_slots<support_half_av_seg>(dci, num_wqes);
    ...
    if (nic_buf_on_gpumem)
        ...
        gic_write_rdma_read_wqe<support_half_av_seg>(dci, dct, laddr, lkey, raddr, rkey, sizeof(char), base_wqe_idx, wqe_ptrs, &ctrl_seg);
#endif
...
template <bool support_half_av_seg, bool nic_buf_on_gpumem>
__device__ static inline uint64_t gic_quiet_with_cst(nvshmemi_gic_device_dci_t *dci, bool is_dci_shared_among_ctas) {
    uint64_t get_head = gic_atomic_read(&dci->mvars->tx_wq.get_head);
    uint64_t ticket = gic_quiet<support_half_av_seg>(dci);
    uint64_t get_tail = gic_atomic_read(&dci->mvars->tx_wq.get_tail);

    // TODO: Change to WAIT + DUMP
    // In that case, we don't have to do quiet first
    if (get_tail < get_head) {
        ticket = gic_cst<support_half_av_seg, nic_buf_on_gpumem>(dci, is_dci_shared_among_ctas);
        gic_update_get_tail(dci, ticket);
    }

    return ticket;
}

template <threadgroup_t SCOPE, nvshmemi_op_t channel_op, bool nbi, bool support_half_av_seg, bool nic_buf_on_gpumem>
__device__ static inline void gic_rma(void *rptr, void *lptr, size_t bytes, int pe) {
    ...
    nvshmemi_gic_device_dci_t *dci = gic_get_dci(pe, &is_dci_shared_among_ctas);

    if (my_tid == 0) {
        int num_wqes = support_half_av_seg ? 1 : 2;
        ...
        switch (channel_op) {
            case NVSHMEMI_OP_PUT:
                gic_write_rdma_write_wqe<support_half_av_seg>(dci, dct, (uint64_t)lptr, lkey, raddr, rkey, bytes, base_wqe_idx, wqe_ptrs, &ctrl_seg);
                break;
            case NVSHMEMI_OP_GET:
                gic_write_rdma_read_wqe<support_half_av_seg>(dci, dct, (uint64_t)lptr, lkey, raddr, rkey, bytes, base_wqe_idx, wqe_ptrs, &ctrl_seg);
                // GET index must be visible before the new cons index.
                // gic_submit_requests has fence, which guarantees the ordering.
                gic_update_get_head(dci, base_wqe_idx + num_wqes);
                break;
            default:
#ifdef NVSHMEM_GPUINITATED_DEBUG
        ...
        if (!nbi) {
            if (need_cst)
                gic_quiet_with_cst<support_half_av_seg, nic_buf_on_gpumem>(dci, is_dci_shared_among_ctas);
            else
                gic_quiet<support_half_av_seg>(dci);
        }
    }
...
__device__ static inline void gic_rma_thread(void *rptr, void *lptr, size_t bytes, int pe) {
    ...
    int num_wqes_per_cmd = support_half_av_seg ? 1 : 2;
    int num_wqes = num_wqes_per_cmd * tg_size;

    uint64_t base_wqe_idx;

    if (my_tid == 0)
        base_wqe_idx = gic_reserve_wqe_slots<support_half_av_seg>(dci, num_wqes);
    ...
    void *wqe_ptrs[2];
    wqe_ptrs[0] = gic_get_wqe_ptr(dci, my_wqe_idx);
    wqe_ptrs[1] = gic_get_wqe_ptr(dci, my_wqe_idx + 1);

    switch (channel_op) {
        case NVSHMEMI_OP_PUT:
            gic_write_rdma_write_wqe<support_half_av_seg>(dci, dct, (uint64_t)lptr, lkey, raddr, rkey, bytes, my_wqe_idx, wqe_ptrs, &ctrl_seg);
            break;
        case NVSHMEMI_OP_GET:
            gic_write_rdma_read_wqe<support_half_av_seg>(dci, dct, (uint64_t)lptr, lkey, raddr, rkey, bytes, my_wqe_idx, wqe_ptrs, &ctrl_seg);
            break;
        default:
#ifdef NVSHMEM_GPUINITIATED_DEBUG
            printf("Unsupported channel_op.\n");
...
__device__ static inline void nvshmemi_gic_rma_p_impl(void *rptr, const T value, int pe) {
    ...
    if (can_combine_data) {
        num_wqes_per_cmd = gic_get_num_wqes_in_inl_combine_warp<T>();
        num_wqes = num_wqes_per_cmd;
    } else {
        num_wqes_per_cmd = support_half_av_seg ? 1 : 2;
    ...
    uint64_t my_wqe_idx = can_combine_data ? base_wqe_idx : base_wqe_idx + (my_tid * num_wqes_per_cmd);

    gic_ctrl_seg_t ctrl_seg;

    assert(likely(num_wqes_per_cmd <= 6));
    void *wqe_ptrs[4];
    wqe_ptrs[0] = gic_get_wqe_ptr(dci, my_wqe_idx);
    wqe_ptrs[1] = gic_get_wqe_ptr(dci, my_wqe_idx + 1);
    wqe_ptrs[2] = gic_get_wqe_ptr(dci, my_wqe_idx + 2);
    wqe_ptrs[3] = gic_get_wqe_ptr(dci, my_wqe_idx + 3);

    if (can_combine_data)
        gic_write_rdma_write_inl_wqe_combine_warp<T>(dci, dct, value, raddr, rkey, my_wqe_idx, my_tid, wqe_ptrs, &ctrl_seg);
    else
        gic_write_rdma_write_inl_wqe<support_half_av_seg>(dci, dct, &value, raddr, rkey, sizeof(T), my_wqe_idx, wqe_ptrs, &ctrl_seg);
...
__device__ inline void nvshmemi_gic_rma_p(void *rptr, const T value, int pe) {
    ...
    if (amask == GIC_FULL_WARP) {
        ...
        can_combine_data = (pred_pe && pred_contiguous && state->support_half_av_seg && sizeof(T) <= 4);
        if (can_combine_data)
...
__device__ inline T nvshmemi_gic_rma_g_impl(void *rptr, int pe) {
    ...
    if (my_tid == 0) {
        base_ibuf_idx = gic_reserve_ibuf_slots(dci, tg_size);
        base_wqe_idx = gic_reserve_wqe_slots<support_half_av_seg>(dci, num_wqes);
    }
    ...
    uint64_t laddr = gic_get_ibuf_addr(dci, my_ibuf_idx);
    __be32 lkey = dci->ibuf.lkey;

    gic_write_rdma_read_wqe<support_half_av_seg>(dci, dct, laddr, lkey, raddr, rkey, sizeof(T), my_wqe_idx, wqe_ptrs, &ctrl_seg);
    ...
    if (my_tid == tg_size - 1)
        gic_release_ibuf(dci, base_ibuf_idx, tg_size);
...
__device__ inline void nvshmemi_gic_amo_nonfetch_impl(void *rptr, const T value, int pe, nvshmemi_amo_t op) {
    unsigned int amask = __activemask();
    int my_tid;
    int tg_size;
    ...
    gic_write_atomic_wqe<support_half_av_seg>(
        dci, dct, &value, NULL, (uint64_t)dci->ibuf.buf, dci->ibuf.lkey,
        raddr, rkey, sizeof(T), my_wqe_idx, op, wqe_ptrs, &ctrl_seg
    );
...
template <typename T>
__device__ inline void nvshmemi_gic_amo_nonfetch(void *rptr, const T value, int pe, nvshmemi_amo_t op) {
    nvshmemi_gic_device_state_t *state = gic_get_state();
 ...
 */
template <typename T, bool support_half_av_seg, bool nic_buf_on_gpumem>
__device__ inline T nvshmemi_gic_amo_fetch_impl(void *rptr, const T value, const T compare, int pe, nvshmemi_amo_t op) {
    unsigned int amask = __activemask();
    int my_tid;
    int tg_size;
...
template <typename T>
__device__ inline T nvshmemi_gic_amo_fetch(void *rptr, const T value, const T compare, int pe, nvshmemi_amo_t op) {
    T ret;
    nvshmemi_gic_device_state_t *state = gic_get_state();
...
__device__ static inline void nvshmemi_gic_put_signal_impl(
    ...
    if (my_tid == 0) {
        int num_rdma_write_wqes = support_half_av_seg ? 1 : 2;
        ...
        wqe_ptrs[0] = gic_get_wqe_ptr(dci, base_wqe_idx);
        wqe_ptrs[1] = gic_get_wqe_ptr(dci, base_wqe_idx + 1);
        wqe_ptrs[2] = gic_get_wqe_ptr(dci, base_wqe_idx + 2);
        wqe_ptrs[3] = gic_get_wqe_ptr(dci, base_wqe_idx + 3);

        gic_write_rdma_write_wqe<support_half_av_seg>(dci, dct, (uint64_t)lptr, lkey, raddr, rkey, bytes, base_wqe_idx, wqe_ptrs, &ctrl_seg);

        gic_write_atomic_wqe<support_half_av_seg>(
            dci, dct, &signal, NULL, (uint64_t)dci->ibuf.buf, dci->ibuf.lkey,
            sig_raddr, sig_rkey, sizeof(signal), base_wqe_idx + num_rdma_write_wqes, sig_op, &wqe_ptrs[num_rdma_write_wqes], 
            &ctrl_seg
        );
...
__device__ static inline void nvshmemi_gic_put_signal_thread_impl(
    void *rptr, void *lptr, size_t bytes
    , void *sig_rptr, uint64_t signal, nvshmemi_amo_t sig_op
    , int pe
) {
    ...
    int num_rdma_write_wqes_per_cmd = support_half_av_seg ? 1 : 2;
    ...
    gic_write_atomic_wqe<support_half_av_seg>(
        dci, dct, &signal, NULL, (uint64_t)dci->ibuf.buf, dci->ibuf.lkey,
        sig_raddr, sig_rkey, sizeof(signal), my_wqe_idx + 1, sig_op, &wqe_ptrs[num_rdma_write_wqes_per_cmd], &ctrl_seg
    );
...
template <threadgroup_t SCOPE>
__device__ inline void nvshmemi_gic_put_signal(
    void *rptr, void *lptr, size_t bytes
    , void *sig_rptr, uint64_t signal, nvshmemi_amo_t sig_op
    , int pe, bool is_nbi
) {
    ...
    if (SCOPE == NVSHMEMI_THREADGROUP_THREAD) {
        if (is_nbi && state->support_half_av_seg)
            nvshmemi_gic_put_signal_thread_impl<true, true>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
        else if (is_nbi && !state->support_half_av_seg)
            nvshmemi_gic_put_signal_thread_impl<true, false>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
        else if (!is_nbi && state->support_half_av_seg)
            nvshmemi_gic_put_signal_thread_impl<false, true>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
        else
            nvshmemi_gic_put_signal_thread_impl<false, false>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
    } else {
        if (is_nbi && state->support_half_av_seg)
            nvshmemi_gic_put_signal_impl<SCOPE, true, true>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
        else if (is_nbi && !state->support_half_av_seg)
            nvshmemi_gic_put_signal_impl<SCOPE, true, false>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
        else if (!is_nbi && state->support_half_av_seg)
            nvshmemi_gic_put_signal_impl<SCOPE, false, true>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
        else
            nvshmemi_gic_put_signal_impl<SCOPE, false, false>(rptr, lptr, bytes, sig_rptr, signal, sig_op, pe);
    }
}
...
__device__ inline void nvshmemi_gic_quiet() {
    ...
    if (amask == GIC_FULL_WARP) {
        for (uint32_t i = nvshmemi_thread_id_in_warp(); i < ndcis; i += warpSize) {
            dci = &state->globalmem.dcis[i];
#ifdef NVSHMEM_GPUINITIATED_SUPPORT_GPUMEM_ONLY
            if (state->support_half_av_seg)
                gic_quiet_with_cst<true, true>(dci, true);
            else
                gic_quiet_with_cst<false, true>(dci, true);
#else
            if (state->support_half_av_seg && state->nic_buf_on_gpumem)
                gic_quiet_with_cst<true, true>(dci, true);
            else if (state->support_half_av_seg && !state->nic_buf_on_gpumem)
                gic_quiet_with_cst<true, false>(dci, true);
            else if (!state->support_half_av_seg && state->nic_buf_on_gpumem)
                gic_quiet_with_cst<false, true>(dci, true);
            else
                gic_quiet_with_cst<false, false>(dci, true);
#endif /* NVSHMEM_GPUINITIATED_SUPPORT_GPUMEM_ONLY */
        }
    
        nvshmemi_warp_sync();
    } else {
        for (uint32_t i = 0; i < ndcis; ++i) {
            dci = &state->globalmem.dcis[i];
#ifdef NVSHMEM_GPUINITIATED_SUPPORT_GPUMEM_ONLY
            if (state->support_half_av_seg)
                gic_quiet_with_cst<true, true>(dci, true);
...
__device__ inline void nvshmemi_gic_enforce_consistency_at_target(bool use_membar) {
    nvshmemi_gic_device_state_t *state = gic_get_state();
    bool is_dci_shared_among_ctas;
    nvshmemi_gic_device_dci_t *dci = gic_get_dci(nvshmemi_device_state_d.mype, &is_dci_shared_among_ctas);
