...
NVSHMEMI_ENV_DEF(
    DISABLE_CUDA_VMM, bool, true, NVSHMEMI_ENV_CAT_OTHER,
...
#else
NVSHMEMI_ENV_DEF(
    DISABLE_CUDA_VMM, bool, false, NVSHMEMI_ENV_CAT_OTHER,
    "Disable use of CUDA VMM for P2P memory mapping. By default, CUDA VMM is enabled "
    "on x86 and disabled on P9. CUDA VMM feature in NVSHMEM requires CUDA RT version and "
    "CUDA Driver version to be greater than or equal to 11.3.")
#endif
...
NVSHMEMI_ENV_DEF(BCAST_LL_THRESHOLD, size, (size_t)(1 << 11), NVSHMEMI_ENV_CAT_COLLECTIVES,
                 "Message size threshold up to which "
...
NVSHMEMI_ENV_DEF(
    DISABLE_LOCAL_ONLY_PROXY, bool, false, NVSHMEMI_ENV_CAT_TRANSPORT,
...
NVSHMEMI_ENV_DEF(
    IBGDA_NUM_RC_PER_PE, int, 0, NVSHMEMI_ENV_CAT_TRANSPORT,
    "Number of RC QPs per peer PE used in GPU-initiated communication transport.\n"
    "Set to 0 to disable RC QPs (default 0).\n"
    "If it is set to a positive number, DCI will be used for enforcing consistency only.")
