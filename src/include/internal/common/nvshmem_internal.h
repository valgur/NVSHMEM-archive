...
static inline bool nvshmemi_is_version_compatible(const nvshmemi_version_t version_host,
    ...
    /* Host is newer, can't interoperate with the older device.
     * For example - a newer host may have added a variable,
     * when it copies into the device, it will copy over the end
     * of device memory.
     */
    if (version_host.minor > version_device.minor) {
        return 1;
    }
