...
class nvshmemi_symmetric_heap {
   public:
    ...
    mspace *get_mspace() { return heap_mspace_; }
...
class nvshmemi_symmetric_heap_static : public nvshmemi_symmetric_heap {
   public:
    ...
    int get_mem_handle_type(void) { return mem_handle_type_; }
    virtual size_t get_registration_chunk_size(void) = 0;
   ...
   private:
    /* Stubbed implementation to not be accessible in derived class */
...
class nvshmemi_symmetric_heap_dynamic : public nvshmemi_symmetric_heap {
   public:
    ...
    virtual void *allocate_symmetric_memory(size_t size, size_t count, size_t alignment,
                                            int type) = 0;
    int get_mem_handle_type(void) { return mem_handle_type_; }
    void set_mem_handle_type(int type) { mem_handle_type_ = (CUmemAllocationHandleType)(type); }
    virtual int register_heap_chunk(nvshmem_mem_handle_t *mem_handle, void *buf, size_t size);
    /**
     * Given a input mem_handle_in, export the buffer backed by the handle to target mem handle
     */
   ...
   private:
    CUmemAllocationHandleType mem_handle_type_ = CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR;
};
...
class nvshmemi_symmetric_heap_vidmem_static_pinned final
   ...
   public:
    ...
    size_t get_registration_chunk_size(void);
...
class nvshmemi_symmetric_heap_vidmem_dynamic_vmm final
   ...
   private:
    ...
    bool is_cuda_mem_handle_type_ipc(int mem_handle_type) const {
        return (mem_handle_type == (int)CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR);
    }
    ...
    std::vector<CUmemGenericAllocationHandle> cumem_handles_;
};
