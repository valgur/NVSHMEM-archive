...
#define NVSHMEMI_GPU_ALLTOALL_THREADGROUP_ALLPUSH(SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE, dest, \
    ...
    do {                                                                                          \
        ...
        for (int ii = 0; ii < PE_size; ii++) {                                                    \
            ...
            nvshmem##SC_PREFIX##_##TYPENAME##_put_nbi##SC_SUFFIX(                                 \
...
#define NVSHMEMI_GPU_ALLTOALL_THREADGROUP_P2P_ALLPUSH(SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE,    \
    ...
    do {                                                                                           \
        ...
        for (int ii = 0; ii < PE_size; ii++) {                                                     \
            ...
            GPU_BITS_COPY_THREADGROUP_DIRECT(TYPENAME, TYPE, dst_ptr, source + src_offset, nelems, \
                                             myIdx, groupSize);                                    \
        }                                                                                          \
        nvshmem##SC_PREFIX##i_barrier##SC_SUFFIX(PE_start, PE_stride, PE_size, pSync, NULL);       \
    } while (0)

#define NVSHMEMI_GPU_ALLTOALL_THREADGROUP(SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE, dest, source, \
                                          nelems, PE_start, PE_stride, PE_size, pSync)            \
    do {                                                                                          \
        if (nvshmemi_device_state_d.job_connectivity <= NVSHMEMI_JOB_GPU_LDST)                    \
            NVSHMEMI_GPU_ALLTOALL_THREADGROUP_P2P_ALLPUSH(SC, SC_SUFFIX, SC_PREFIX, TYPENAME,     \
                                                          TYPE, dest, source, nelems, PE_start,   \
                                                          PE_stride, PE_size, pSync);             \
        else                                                                                      \
            NVSHMEMI_GPU_ALLTOALL_THREADGROUP_ALLPUSH(SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE,   \
                                                      dest, source, nelems, PE_start, PE_stride,  \
                                                      PE_size, pSync);                            \
    } while (0)
...
#define DEFN_NVSHMEMX_TYPENAME_ALLTOALL_THREADGROUP(SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE)     \
    __device__ int nvshmemx_##TYPENAME##_alltoall##SC_SUFFIX(nvshmem_team_t team, TYPE *dest,     \
                                                             const TYPE *source, size_t nelems) { \
        nvshmemi_team_t *teami = nvshmemi_device_state_d.team_pool[team];                         \
        nvshmem##SC_PREFIX##_barrier##SC_SUFFIX(team);                                            \
        nvshmemxi_##TYPENAME##_alltoall##SC_SUFFIX(dest, source, nelems, teami->start,            \
                                                   teami->stride, teami->size,                    \
...
#define DEFN_NVSHMEM_TYPENAME_ALLTOALL(TYPENAME, TYPE)                                          \
    __device__ int nvshmem_##TYPENAME##_alltoall(nvshmem_team_t team, TYPE *dest,               \
                                                 const TYPE *source, size_t nelems) {           \
        nvshmemi_team_t *teami = nvshmemi_device_state_d.team_pool[team];                       \
        nvshmem_barrier(team);                                                                  \
        nvshmemxi_##TYPENAME##_alltoall(dest, source, nelems, teami->start, teami->stride,      \
