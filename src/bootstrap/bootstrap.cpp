...
int bootstrap_init(int mode, bootstrap_attr_t *attr, bootstrap_handle_t *handle) {
    ...
    if (nvshmemi_options.BOOTSTRAP_PLUGIN_provided) plugin_name = nvshmemi_options.BOOTSTRAP_PLUGIN;

    switch (mode) {
        case BOOTSTRAP_MPI:
            if (!plugin_name) plugin_name = BOOTSTRAP_MPI_PLUGIN;
        ...
        case BOOTSTRAP_SHMEM:
            if (!plugin_name) plugin_name = BOOTSTRAP_SHMEM_PLUGIN;
        ...
        case BOOTSTRAP_PMI:
            if (strcmp_case_insensitive(nvshmemi_options.BOOTSTRAP_PMI, "PMIX") == 0) {
                if (!plugin_name) plugin_name = BOOTSTRAP_PMIX_PLUGIN;
